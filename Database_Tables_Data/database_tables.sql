...
CREATE TABLE IF NOT EXISTS `mydb`.`Library_Manager` (
  ...
  `password` VARCHAR(45) NOT NULL,
  `approved` VARCHAR(10) NOT NULL CHECK (`approved` IN ('rejected','pending', 'accepted')),
...
CREATE TABLE IF NOT EXISTS `mydb`.`School_User` (
  ...
  `approved` VARCHAR(10) NOT NULL CHECK (`approved` IN ('rejected','pending', 'accepted', 'suspended')),
...
CREATE TABLE IF NOT EXISTS `mydb`.`Review` (
  ...
  `approved` VARCHAR(100) NOT NULL CHECK (`approved` IN ('rejected', 'pending', 'accepted', 'not needed')) ,
  `username_libm` VARCHAR(45) NOT NULL,
