...
SET @t1 = 'SELECT DISTINCT b.title, bd.state, su.username_su 
        FROM book b
...
SET @t1 = CONCAT(@t1, ' AND bd.state IN ("active", "overdue", "returned")  AND bd.category = ''B'' ');
SET @t1 = CONCAT(@t1, ' GROUP BY b.title, su.username_su, bd.state');
